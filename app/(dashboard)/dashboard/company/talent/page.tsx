        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex gap-2 flex-wrap">
                    {SPECIALIZATIONS.map(s => (
                        <button key={s} onClick={() => setSpec(s)}
                            className={`px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${spec === s ? "bg-primary text-navy-950 border-primary" : "border-navy-600 text-navy-400 hover:text-white"}`}>
                            {s}
                        </button>
                    ))}
                </div>
                <div className="relative">
                    ...
                    <input placeholder="Search engineers..." className="pl-10 h-9 bg-navy-800 border border-navy-600 rounded-xl text-sm text-white placeholder:text-navy-500 focus:border-primary focus:outline-none w-56" value={search} onChange={e => setSearch(e.target.value)} />
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {filtered.map((eng, idx) => (
                    <motion.div key={eng.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }}
                        className={`relative bg-navy-900 border border-navy-700 rounded-xl p-5 hover:border-primary/20 transition-colors ${eng.locked ? "overflow-hidden" : ""}`}>
                        {eng.locked && <div className="absolute inset-0 backdrop-blur-[2px] bg-navy-900/30 z-10 flex items-center justify-center">
                            <div className="text-center">
                                <Lock className="w-6 h-6 text-navy-500 mx-auto mb-2" />
                                <p className="text-xs text-navy-400">Upgrade to view</p>
                            </div>
                        </div>}
                        <div className="flex items-center gap-3 mb-3">
                            ...
                            <div>
                                <p className="font-semibold text-sm text-white">{eng.name}</p>
                                <p className="text-xs text-navy-400">{eng.spec} · {eng.experience}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-4">
                            {eng.skills.map(s => (
                                <span key={s} className="text-[10px] px-2 py-0.5 rounded-full bg-navy-800 text-navy-300 border border-navy-700">{s}</span>
                            ))}
                        </div>
