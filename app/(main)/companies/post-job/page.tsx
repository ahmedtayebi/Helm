                    ...
                    <React.Fragment key={label}>
                        <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold border-2 transition-all duration-300 ${
                                ...
                                : 'border-navy-600 text-navy-500 bg-navy-900'
                            }`}>
                                {idx < step ? <CheckCircle className="w-4 h-4" /> : idx + 1}
                            </div>
                            <span className={`hidden md:block text-sm font-medium ${idx <= step ? 'text-white' : 'text-navy-500'}`}>{label}</span>
                        </div>
                        {idx < STEPS.length - 1 && (
                            <div className={`flex-1 mx-3 h-px transition-all duration-500 ${idx < step ? 'bg-primary' : 'bg-navy-700'}`} />
                        )}
                    </React.Fragment>
                ))}
            </div>
...
function JobPreviewCard({ form }: { form: FormData }) {
    ...
    return (
        <div className="bg-navy-900 border border-primary/30 rounded-2xl p-5 shadow-gold-sm relative overflow-hidden">
            ...
            <div className="flex items-start gap-3 mb-3">
                <div className="w-10 h-10 rounded-lg bg-navy-800 border border-navy-600 flex items-center justify-center font-bold text-primary text-sm flex-shrink-0">
                    {(form.companyName || "CO").charAt(0)}
                </div>
                <div>
                    <h3 className="font-display text-base text-white leading-tight">{form.title || "Job Title"}</h3>
                    <p className="text-navy-400 text-xs">{form.companyName || "Your Company"}</p>
                </div>
            </div>
            <div className="flex flex-wrap gap-1.5 mb-3 text-xs">
                {form.location && (
                    <span className="flex items-center gap-1 text-navy-400"><MapPin className="w-3 h-3"/>{form.location}</span>
                )}
                <span className={`px-2 py-0.5 rounded-full border font-medium ${typeStyle.bg} ${typeStyle.text} ${typeStyle.border}`}>
                    {form.type || "Full-time"}
                </span>
                {form.experienceLevel && (
                    <span className="px-2 py-0.5 rounded-full bg-navy-800 border border-navy-700 text-navy-300">{form.experienceLevel}</span>
                )}
            ...
            {form.deadline && (
                <div className="text-xs text-navy-500 flex items-center gap-1"><Calendar className="w-3 h-3"/> Closes {new Date(form.deadline).toLocaleDateString("en-GB", { day: "numeric", month: "short" })}</div>
            )}
        </div>
    );
}
...
export default function PostJobPage() {
    ...
    const fieldClass = "w-full bg-navy-800 border border-navy-600 rounded-xl px-4 py-3 text-white placeholder:text-navy-500 text-sm focus:outline-none focus:border-primary transition-colors";
    const labelClass = "block text-sm font-medium text-navy-300 mb-1.5";

    if (published) {
        return (
            <main className="min-h-screen bg-navy-950 pt-24 flex items-center justify-center px-4">
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="max-w-md w-full text-center bg-navy-900 border border-primary/30 rounded-3xl p-10">
                    ...
                    <h2 className="font-display text-3xl text-white mb-3">Job Published!</h2>
                    <p className="text-navy-300 mb-2">Your listing for <strong className="text-white">{form.title}</strong> is now live on HELM.</p>
                    <p className="text-sm text-navy-500 mb-8">Qualified candidates will start applying within 24 hours.</p>
    ...
    return (
        <main className="min-h-screen bg-navy-950 pt-24 pb-16">
            <div className="max-w-6xl mx-auto px-4 xl:px-8">
                ...
                <div className="mb-8">
                    <h1 className="font-display text-4xl text-white">Post a Job</h1>
                    <p className="text-navy-300 mt-2">Reach 12,400+ qualified petroleum engineers in minutes</p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-8">
                    {/* Form */}
                    <div className="bg-navy-900 border border-navy-600 rounded-2xl p-6 md:p-8">

                        {/* ── STEP 1: Job Details ── */}
                        {step === 0 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                                <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-2"><Briefcase className="w-5 h-5 text-primary"/>Job Details</h2>
                                <div>
                                    <label className={labelClass}>Job Title *</label>
                                    <Input required placeholder="e.g. Senior Drilling Engineer" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.title} onChange={e => set("title", e.target.value)} />
                                </div>
                                <div>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <Input placeholder="e.g. Hassi Messaoud, Algeria" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.location} onChange={e => set("location", e.target.value)} />
                                    </div>
                                    <div>
                                ...
                                <label className="flex items-center gap-3 cursor-pointer">
                                    <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${form.isRemote ? 'bg-teal-500 border-teal-500' : 'bg-navy-900 border-navy-600'}`}
                                    ...
                                    <span className="text-sm text-navy-300">Remote / Hybrid work accepted</span>
                                </label>
                                <div>
                                    <label className={labelClass}>Application Deadline</label>
                                    <Input type="date" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.deadline} onChange={e => set("deadline", e.target.value)} />
                                </div>
                                <div>
                                    <label className="flex items-center gap-3 cursor-pointer mb-4">
                                        <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${form.showSalary ? 'bg-primary border-primary' : 'bg-navy-900 border-navy-600'}`}
                                        ...
                                        <span className="text-sm font-medium text-navy-300">Show salary range (increases applications by 40%)</span>
                                    </label>
                                    {form.showSalary && (
                                        <div className="grid grid-cols-2 gap-4">
                                            <div>
                                                <label className="text-xs text-navy-400 mb-1 block">Min</label>
                                                <Input placeholder="e.g. 150,000 DZD" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.salaryMin} onChange={e => set("salaryMin", e.target.value)} />
                                            </div>
                                            <div>
                                                <label className="text-xs text-navy-400 mb-1 block">Max</label>
                                                <Input placeholder="e.g. 200,000 DZD" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.salaryMax} onChange={e => set("salaryMax", e.target.value)} />
                                            </div>
                                        </div>
                                    )}
                                </div>
                        ...
                        {step === 1 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                                <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-2"><CheckCircle className="w-5 h-5 text-primary"/>Requirements & Responsibilities</h2>
                                <div>
                                    <label className={labelClass}>Requirements *</label>
                                    <p className="text-xs text-navy-500 mb-2">One per line (e.g. &ldquo;5+ years drilling experience&rdquo;)</p>
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-navy-500 mb-2">What will the engineer do day-to-day?</p>
                        ...
                        {step === 2 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-5">
                                <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-2"><Building2 className="w-5 h-5 text-primary"/>Company Information</h2>
                                <div>
                                    <label className={labelClass}>Company Name *</label>
                                    <Input required placeholder="e.g. Sonatrach" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.companyName} onChange={e => set("companyName", e.target.value)} />
                                </div>
                                <div>
                                    <label className={labelClass}>Company Website</label>
                                    <Input placeholder="https://www.yourcompany.com" className="bg-navy-800 border-navy-600 focus:border-primary" value={form.companyWebsite} onChange={e => set("companyWebsite", e.target.value)} />
                                </div>
                                <div>
                        ...
                        {step === 3 && (
                            <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                <h2 className="font-display text-2xl text-white mb-6 flex items-center gap-2"><FileText className="w-5 h-5 text-primary"/>Review & Publish</h2>
                                <div className="space-y-4 mb-8">
                                    {[
                                        ...
                                        <div key={label} className="flex justify-between items-center py-3 border-b border-navy-700">
                                            <span className="text-sm text-navy-400">{label}</span>
                                            <span className="text-sm text-white font-medium">{val || <span className="text-navy-600 italic">Not set</span>}</span>
                                        </div>
                                    ))}
                                </div>
                    ...
                    <div className="hidden lg:block">
                        <div className="sticky top-24">
                            <p className="text-xs text-navy-500 uppercase tracking-wider font-medium mb-3">Live Card Preview</p>
                            <JobPreviewCard form={form} />
                            <div className="mt-4 bg-navy-900 border border-navy-600 rounded-xl p-4 text-xs text-navy-400 space-y-2">
