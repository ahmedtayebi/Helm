...
function LibrarySearchContent({ initialResources }: Props) {
    ...
    const filteredResources = useMemo(() => {
        ...
        return result;
    }, [searchQuery, activeFilters, sortBy, isFeatured, initialResources]);
            ...
            <div className="max-w-7xl mx-auto px-6">
                ...
                <div className="mb-10 mt-4">
                    ...
                    <p className="text-slate-500 dark:text-navy-300">
                        Browse and filter thousands of items in the HELM digital library.
                    </p>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-10">
                    ...
                    <aside className={`lg:w-72 lg:flex-shrink-0 space-y-8 ${isMobileFiltersOpen ? "block" : "hidden lg:block"}`}>
                        <div className="glass p-6 rounded-2xl border border-slate-200 dark:border-navy-600 sticky top-28">
                            <div className="flex items-center justify-between mb-6">
                                ...
                                {totalActiveFilters > 0 && (
                                    <button
                                        ...
                                        className="text-sm text-slate-500 dark:text-navy-300 hover:text-primary transition-colors"
                                    >
                                        Clear all
                                    </button>
                                )}
                            ...
                            <div className="mb-8">
                                <h3 className="text-sm font-bold text-[#0D1B2A] dark:text-white uppercase tracking-wider mb-4">
                                    Access Level
                                </h3>
                                <div className="flex gap-3">
                                    {["Free", "Premium"].map((p) => (
                                        <button
                                            ...
                                            className={`flex flex-1 items-center justify-center gap-1.5 py-2 text-sm font-medium rounded-lg border transition-all ${
                                                activeFilters.access.includes(p)
                                                    ? p === "Premium"
                                                        ? "bg-gradient-gold shadow-gold-sm border-yellow-200/50 text-navy-950"
                                        ...
                                        >
                                            {p === "Premium" && (
                                                <Lock className={`w-3.5 h-3.5 ${activeFilters.access.includes(p) ? "text-navy-950" : "text-primary"}`} />
                                            )}
                                            {p}
                                        </button>
                                    ))}
                                    ...
                                    {["AR", "EN", "FR"].map((lang) => (
                                        <button
                                            ...
                                            className={`px-4 py-1.5 text-sm font-medium rounded-lg border transition-all ${
                                                activeFilters.language.includes(lang)
                            ...
                            <div className="relative w-full sm:max-w-xl">
                                ...
                                {searchQuery && (
                                    <button
                                        ...
                                        className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-navy-400 hover:text-[#0D1B2A] dark:hover:text-white bg-slate-200 dark:bg-navy-800 p-1 rounded"
                                    >
                                        <X className="w-4 h-4" />
                                    </button>
                                )}
                                    ...
                                    {Object.entries(activeFilters).map(([type, values]) =>
                                        values.map((val) => (
                                            <Badge
                                                key={`${type}-${val}`}
                                                variant="outline"
                                                className="flex items-center gap-1.5 bg-white/50 dark:bg-navy-900/50 border-primary/30 text-[#0D1B2A] dark:text-white"
                                            >
                                                {val}
                                                <button
                            ...
                            <AnimatePresence mode="popLayout">
                                {filteredResources.length > 0 ? (
                                    ...
                                    >
                                        ...
                                        <h3 className="text-2xl text-[#0D1B2A] dark:text-white font-display mb-2 drop-shadow-md">
                                            No resources found
                                        </h3>
                        ...
                        {filteredResources.length > 0 && (
                            <div className="mt-12 flex justify-center">
                                <div className="flex gap-2">
                                    <Button variant="outline" size="sm" disabled className="border-slate-300 dark:border-navy-600 text-slate-400 dark:text-navy-500">
                                        Previous
                                    </Button>
...
// ── Suspense shell needed for useSearchParams() ───────────────
export function LibrarySearchShell({ initialResources }: Props) {
    return (
        <Suspense
            fallback={
        ...
        >
            <LibrarySearchContent initialResources={initialResources} />
        </Suspense>
    );
}
                    ...
                    >
                        <div
                            className={`w-5 h-5 rounded border flex items-center justify-center transition-all duration-200 ${
                                active.includes(opt)
                                    ? "bg-primary border-primary shadow-[0_0_10px_rgba(45,212,191,0.5)]"
                                    : "bg-white dark:bg-navy-900 border-slate-300 dark:border-navy-500 group-hover:border-primary/50"
                            }`}
                        >
                        ...
                        <span
                            className={`text-sm ${
                                active.includes(opt)
                                    ? "text-[#0D1B2A] dark:text-white font-medium"
                                    : "text-slate-500 dark:text-navy-300 group-hover:text-[#0D1B2A] dark:group-hover:text-white transition-colors"
                            }`}
                        >
                            {opt}
                        </span>
