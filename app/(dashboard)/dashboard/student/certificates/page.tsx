    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <p className="text-sm text-slate-500 dark:text-navy-400">{CERTIFICATES.length} certificates earned</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {CERTIFICATES.map((cert, idx) => (
                    <motion.div key={cert.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.06 }}
                        className="relative bg-white dark:bg-navy-900 border border-slate-200 dark:border-navy-700 rounded-2xl p-5 hover:border-primary/30 transition-colors group overflow-hidden">
                        ...
                        <div className="flex items-start gap-3 mb-4">
                            ...
                            <div className="min-w-0">
                                <p className="font-display text-base text-[#0D1B2A] dark:text-white truncate">{cert.title}</p>
                                <p className="text-xs text-slate-500 dark:text-navy-400">{cert.course}</p>
                            </div>
                        </div>
                        <div className="flex items-center justify-between text-xs text-slate-400 dark:text-navy-500 mb-4">
                            <span>Issued {cert.issueDate}</span>
                            <span className="font-mono text-slate-400 dark:text-navy-600">{cert.credentialId}</span>
                        </div>
