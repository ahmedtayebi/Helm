...
export default function UploadCoursePage() {
    ...
    const fieldClass = "w-full bg-navy-800 border border-navy-600 rounded-xl px-4 py-3 text-sm text-white placeholder:text-navy-500 focus:border-primary focus:outline-none transition-colors";
        ...
        return (
            <div className="max-w-md mx-auto text-center py-20">
                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
                    ...
                    <h2 className="font-display text-3xl text-white mb-3">Course Published! 🎉</h2>
                    <p className="text-navy-300 mb-6">Your course &ldquo;{form.title || "New Course"}&rdquo; is now live on HELM Academy.</p>
    ...
    return (
        <div className="max-w-2xl mx-auto space-y-6">
                ...
                {STEPS.map((label, idx) => (
                    <React.Fragment key={label}>
                        <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all ${idx < step ? "bg-primary border-primary text-navy-950" : idx === step ? "border-primary text-primary bg-primary/10" : "border-navy-600 text-navy-500"}`}>
                                {idx < step ? <CheckCircle className="w-4 h-4" /> : idx + 1}
                            </div>
                            <span className={`hidden md:block text-xs font-medium ${idx <= step ? "text-white" : "text-navy-500"}`}>{label}</span>
                        </div>
            ...
            <div className="bg-navy-900 border border-navy-700 rounded-2xl p-6 md:p-8">
                <AnimatePresence mode="wait">
                    {/* Step 1 */}
                    {step === 0 && (
                        <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            <h2 className="font-display text-xl text-white flex items-center gap-2"><FileText className="w-5 h-5 text-primary" />Course Details</h2>
                    ...
                    {step === 1 && (
                        <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            <h2 className="font-display text-xl text-white flex items-center gap-2"><List className="w-5 h-5 text-primary" />Curriculum</h2>
                    ...
                    {step === 2 && (
                        <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            <h2 className="font-display text-xl text-white flex items-center gap-2"><DollarSign className="w-5 h-5 text-primary" />Pricing</h2>
                            <label className="flex items-center gap-3 cursor-pointer">
                                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${form.isFree ? "bg-teal-500 border-teal-500" : "bg-navy-800 border-navy-600"}`} onClick={() => set("isFree", !form.isFree)}>
                                ...
                                <span className="text-sm text-navy-300">This is a free course</span>
                            </label>
                            {!form.isFree && (
                            ...
                            <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 text-sm text-navy-300">
                    ...
                    {step === 3 && (
                        <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
                            <h2 className="font-display text-xl text-white flex items-center gap-2"><Send className="w-5 h-5 text-primary" />Review & Publish</h2>
                            <div className="space-y-3">
                                {[["Title", form.title], ["Specialization", form.specialization], ["Level", form.level], ["Language", form.language], ["Price", form.isFree ? "Free" : `$${form.price || "—"}`]].map(([l, v]) => (
                                    <div key={l} className="flex justify-between py-2 border-b border-navy-800">
                                        <span className="text-sm text-navy-400">{l}</span>
                                        <span className="text-sm text-white font-medium">{v || "Not set"}</span>
                                    </div>
                                ))}
                            </div>
                            <div className="bg-teal-500/5 border border-teal-500/20 rounded-xl p-4 text-sm text-navy-300">
