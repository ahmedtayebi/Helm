    ...
    return (
        <div className="space-y-6">
            <div className="relative max-w-sm">
                ...
                <input placeholder="Search by name or course..." className="pl-10 h-10 w-full bg-navy-800 border border-navy-600 rounded-xl text-sm text-white placeholder:text-navy-500 focus:border-primary focus:outline-none" value={search} onChange={e => setSearch(e.target.value)} />
            </div>
                ...
                <table className="w-full text-sm">
                    <thead>
                        <tr className="border-b border-navy-700">
                            <th className="text-left px-4 py-3 text-xs text-navy-400 font-medium">Student</th>
                            <th className="text-left px-4 py-3 text-xs text-navy-400 font-medium hidden md:table-cell">Course</th>
                            <th className="text-center px-2 py-3 text-xs text-navy-400 font-medium">Progress</th>
                            <th className="text-right px-4 py-3 text-xs text-navy-400 font-medium hidden sm:table-cell">Enrolled</th>
                            <th className="px-4 py-3"></th>
                        </tr>
                    </thead>
                    <tbody>
                        {filtered.map((s, idx) => (
                            <tr key={s.id} className={`border-b border-navy-800 hover:bg-navy-800/30 transition-colors ${idx % 2 === 1 ? 'bg-navy-900/40' : ''}`}>
                                <td className="px-4 py-3">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-[10px] text-navy-500">{s.email}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-4 py-3 text-navy-300 text-xs hidden md:table-cell">{s.course}</td>
                                <td className="px-2 py-3">
                                    <div className="flex items-center justify-center gap-2">
                                        <div className="w-20 h-1.5 bg-navy-800 rounded-full">
                                            <div className={`h-full rounded-full ${s.progress === 100 ? 'bg-teal-400' : 'bg-primary'}`} style={{ width: `${s.progress}%` }} />
                                        </div>
                                        <span className="text-xs text-navy-300 w-8">{s.progress}%</span>
                                    </div>
                                </td>
                                <td className="px-4 py-3 text-xs text-navy-400 text-right hidden sm:table-cell">{s.enrolled}</td>
                                <td className="px-4 py-3">
