        ...
        <div className="space-y-6">
            ...
            <motion.div {...fadeUp} className="relative bg-gradient-to-r from-primary/15 via-primary/5 to-transparent border border-primary/20 rounded-2xl p-6 overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <h1 className="font-display text-2xl md:text-3xl text-white mb-1">Welcome back, Ahmed! 🔥</h1>
                    <p className="text-navy-300">Keep your streak alive — You have 4 courses in progress.</p>
                </div>
            </motion.div>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2 space-y-4">
                    ...
                    {COURSES_IN_PROGRESS.map((course, idx) => (
                        <motion.div key={course.id} {...fadeUp} transition={{ delay: 0.1 + idx * 0.08 }}
                            ...
                            <div className="w-14 h-14 rounded-xl bg-navy-800 border border-navy-700 flex items-center justify-center text-2xl flex-shrink-0">
                                {course.thumbnail}
                            </div>
                            <div className="flex-1 min-w-0">
                                <p className="text-sm font-semibold text-white truncate">{course.title}</p>
                                <p className="text-xs text-navy-400 mb-2">{course.instructor} · {course.category}</p>
                            ...
                            <div className="flex flex-col items-end gap-2">
                                <span className="text-[10px] text-navy-500 flex items-center gap-1"><Clock className="w-3 h-3" />{course.lastAccess}</span>
                ...
                <div className="space-y-6">
                    {/* Upcoming Events */}
                    <div>
                        <h2 className="font-display text-lg text-white mb-4">Upcoming Events</h2>
                        <div className="space-y-3">
                            {EVENTS.map((ev, idx) => (
                                <motion.div key={ev.title} {...fadeUp} transition={{ delay: 0.2 + idx * 0.08 }}
                                    className="bg-navy-900 border border-navy-700 rounded-xl p-3 hover:border-primary/20 transition-colors">
                                    <div className="flex items-start gap-3">
                                        ...
                                        <div className="min-w-0">
                                            ...
                                            <p className="text-xs text-navy-400">{ev.date} · {ev.time}</p>
                        ...
                        <div className="bg-navy-900 border border-navy-700 rounded-xl divide-y divide-navy-800">
                            {RECENT_ACTIVITY.map((act, idx) => (
                                <div key={idx} className="flex items-center gap-3 px-4 py-3">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-xs text-navy-200 truncate">{act.text}</p>
                                        <p className="text-[10px] text-navy-500">{act.time}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
