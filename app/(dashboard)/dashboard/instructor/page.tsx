        ...
        <div className="space-y-6">
            ...
            <motion.div {...fadeUp} transition={{ delay: 0.15 }}
                ...
                <div className="flex items-center justify-between mb-4">
                    <h2 className="font-display text-lg text-white">Revenue Overview</h2>
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                ...
                <div className="xl:col-span-2">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="font-display text-lg text-white">Course Performance</h2>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-navy-700">
                                    <th className="text-left px-4 py-3 text-xs text-navy-400 font-medium">Course</th>
                                    <th className="text-center px-2 py-3 text-xs text-navy-400 font-medium hidden sm:table-cell">Students</th>
                                    <th className="text-center px-2 py-3 text-xs text-navy-400 font-medium hidden md:table-cell">Rating</th>
                                    <th className="text-center px-2 py-3 text-xs text-navy-400 font-medium">Completion</th>
                                    <th className="text-right px-4 py-3 text-xs text-navy-400 font-medium">Revenue</th>
                                </tr>
                            </thead>
                            <tbody>
                                {COURSE_PERFORMANCE.map((c, idx) => (
                                    <tr key={c.title} className={`border-b border-navy-800 hover:bg-navy-800/50 transition-colors ${idx % 2 === 1 ? 'bg-navy-900/40' : ''}`}>
                                        <td className="px-4 py-3 font-medium text-white truncate max-w-[200px]">{c.title}</td>
                                        <td className="text-center px-2 py-3 text-navy-300 hidden sm:table-cell">{c.students}</td>
                                        ...
                                        <td className="text-center px-2 py-3">
                                            <div className="flex items-center justify-center gap-2">
                                                <div className="w-16 h-1.5 bg-navy-800 rounded-full">
                                                    <div className="h-full bg-primary rounded-full" style={{ width: `${c.completion}%` }} />
                                                </div>
                                                <span className="text-xs text-navy-300">{c.completion}%</span>
                                            </div>
                                        </td>
                ...
                {/* Recent Enrollments */}
                <div>
                    <h2 className="font-display text-lg text-white mb-4">Recent Enrollments</h2>
                    <div className="bg-navy-900 border border-navy-700 rounded-xl divide-y divide-navy-800">
                        {RECENT_ENROLLMENTS.map((e) => (
                            <div key={e.name + e.course} className="flex items-center gap-3 px-4 py-3 hover:bg-navy-800/30 transition-colors">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-[10px] text-navy-400 truncate">{e.course}</p>
                                </div>
                                <span className="text-[10px] text-navy-500 flex items-center gap-1 flex-shrink-0"><Clock className="w-3 h-3" />{e.time}</span>
                            </div>
                        ))}
                    </div>
                </div>
